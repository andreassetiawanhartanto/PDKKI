  ...
  // ****************** Output **********************/
	 ...
	 if ($num_of_trs > (int)$leading)
	 {
		 for($i=$leading;$i<$num_of_trs;$i++) {
			for($z=0;$z<$num_of_cols;$z++) {						
				if(isset($rows[$i])) {										
		     		echo '<div class="mod_placehere_following ' . $class . '">' . $rows[$i] . '</div>'; 			
				} else {
				 	echo '<div class="mod_placehere_following ' . $class . '">&nbsp;</div>';
				}			
				$i++;			
			} 
			//echo '<br style="clear:both" />';	
			$i--;
			if($class == "even clearfix") {
				$class = "odd clearfix";
			} else {
				$class = "even clearfix";
			}
