...
CKEDITOR.dialog.add('jtreelink',function(editor)
{
	...
	var parseLink = function (editor, element) 
	{
		...
		{
			document.forms[formname].url.value = urlMatch[1];
		}
		...
		if(element)
		{
			...
			document.forms[formname].url.value = element.getAttribute('href');
			document.forms[formname].text.value = element.getText();
			document.forms[formname].title.value = element.getAttribute('title'); 
		}
	...
	var treeId =   CKEDITOR.tools.getNextNumber();
	
	var formname = 'ckadminForm_' + treeId ; 	
	...
	return {
		...
		[
		 	{
				...
				[
					{
						type:	'html',
						html:	'<div id="jtree-content_tree_' +treeId+'" style="height:330px;width:590px;overflow:auto;border:1px solid #CCC;"></div>' +
		...
		onLoad : function()
		{
			...
			tree =  new MooTreeControl({ 
				div: 'jtree-content_tree_' + treeId, 
				mode: 'files',
				grid: true,
				...
				onSelect: function (node,state){
					...
					{
						document.forms[formname].url.value = node.data.url;
						document.forms[formname].text.value = node.text;
						document.forms[formname].title.value = node.text;
					}
				}
		...
		onShow : function ()
		{
			...
			document.forms[formname].url.value = '';
			document.forms[formname].text.value = '';
			document.forms[formname].title.value = ''; 
		...
		onOk : function()
		{
			...
			var url = document.forms[formname].url.value || false;
			
			var linkText = document.forms[formname].text.value;
